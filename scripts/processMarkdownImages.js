...
async function processMarkdownFiles() {
  try {
    ...
    console.log("Detected markdown files:", filenames.filter(f => f.endsWith('.md')));
    ...
    for (const filename of filenames) {
      ...
      if (images.length === 0) {
        ...
        console.log(`Found ${images.length} images in ${filename}:`, images.map(m => m[1]));
      }
      ...
      for (const match of images) {
        ...
        console.log(`Processing image link '${originalSyntax}' in file '${filename}'`);
        ...
        try {
          ...
          if (fs.existsSync(imageSourcePath)) {
            ...
            console.log(`Copied image from ${imageSourcePath} to ${imageDestPath}`);
          } else {
